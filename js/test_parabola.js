(function(){
    ...
    u[128][128]=1;
    const nabla = gpu.createKernel(function(a,c,dt,m1) {
    ...
    const integral=gpu.createKernel(function(a,b,dt,m1) {
        return (a[this.thread.x][this.thread.y]+b[this.thread.x][this.thread.y]*dt)*m1[this.thread.x][this.thread.y];
    }).setOutput([256, 256]);
    var n=create2d();
    var n1;
  ...
  .setGraphical(true);
    var count=0;
    var mur=(deltatime - 1) / (deltatime + 1);
    loop();
    function loop(){
        count++;
        u[128][64]=Math.sin(count*deltatime);
        var n1=nabla(u,csq,deltatime,mask);
        for(var i=0;i<254;i++){
            for(var j=0;j<254;j++){
                n[i+1][j+1]=n1[i][j];
            }
        }
        dudt=integral(dudt,n,deltatime,mask);
        var u1=integral(u,dudt,deltatime,mask);
        for(var i=0;i<256;i++){
            u1[0][i]=u[1][i]+mur*(u1[1][i]-u[0][i]);
            u1[255][i]=u[254][i]+mur*(u1[254][i]-u[255][i]);
            u1[i][0]=u[i][1]+mur*(u1[i][1]-u[i][0]);
            u1[i][255]=u[i][254]+mur*(u1[i][254]-u[i][255]);
        }
        u=u1;
        graphic(u,1);
